...
int restore_secret2(int n, TwoD (&AA), mpz_t b[])
{
...
int restore_secret(int n, mpz_t AA[][opt_threshold], mpz_t b[])
{
    //mpz_t* AA[n] = new mpz_t* [n];
...
void split(void)
{
  ...
  char buf[MAXLINELEN] = "hello world";
  int deg, i;
  for(fmt_len = 1, i = opt_number; i >= 10; i /= 10, fmt_len++);
  if (! opt_quiet) {
    printf("\nGenerating shares using a (%d,%d) scheme with ",
...
void combine(void)
{
  ...
  char buf[MAXLINELEN];
    ...
    char* buffeed[opt_threshold];
    buffeed[0] = "5-b0ba2ba7114726d651a4ae";
    buffeed[1] = "4-eaaa7f024c61f915ab8d20";
